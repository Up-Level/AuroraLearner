...
offset = np.deg2rad(
    np.array([data["INST_AZIMUTH"], data["INST_CO_ELEVATION"], data["INST_ROLL"]])
)
...
for i in range(wic_pixels.shape[0]):
    ...
    scsv = np.array([data["SCSV_X"][i], data["SCSV_Y"][i], data["SCSV_Z"][i]])
    sc = np.array([data["SV_X"][i], data["SV_Y"][i], data["SV_Z"][i]])
    ...
    image_cart = (
        (pixel_cart - pixel_cart.min())
        / (pixel_cart.max() - pixel_cart.min() + 0.00001)
        * 255
    ).astype(np.uint8)

    image_latlon = (
        (pixel_latlon - pixel_latlon.min())
        / (pixel_latlon.max() - pixel_latlon.min() + 0.00001)
        * 255
    ).astype(np.uint8)

    image = (
        (current_image - current_image.min())
        / (current_image.max() - current_image.min() + 0.00001)
        * 255
    ).astype(np.uint8)
